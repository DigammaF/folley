...
impl Scope {
    ...
    fn bind_repr(&mut self, identifier: Identifier, repr: String) {
        if self.rev_reprs.contains_key(&repr) { panic!("{repr} ({identifier}) already bound"); }
    ...
    fn parse(source: &str) -> Result<Self, String> {
        if let Some(captures) = INSTANTIATE_RE.captures(source) {
            ...
            if let Ok(key) = captures["theorem"].parse::<usize>() {
                return Ok(Operation::Instantiate(key, values));
            } else { return Err(format!("Cannot parse {} as a key", &captures["theorem"]).to_string()); }

        } else if let Some(captures) = MODUS_PONENS_RE.captures(source) {
            
            if let Ok(key) = captures["theorem"].parse::<usize>() {
                return Ok(Operation::ModusPonens(key));
            } else { return Err(format!("Cannot parse {} as a key", &captures["theorem"]).to_string()); }
            ...
            return Ok(Operation::Name(values));
        
        } else if let Some(captures) = REWRITE_RE.captures(source) {

            if let Ok(key) = captures["theorem"].parse::<usize>() {
                return Ok(Operation::Rewrite(key));
            } else { return Err(format!("Cannot parse {} as a key", &captures["theorem"]).to_string()); } 

        } else if EVAL_RE.is_match(&source) {

            return Ok(Operation::Eval);

        } else if QED_RE.is_match(&source) {

            return Ok(Operation::QED);

        } else if CONTRADICT_RE.is_match(&source) {

            return Ok(Operation::Contradict);

        } else { return Err("Couldn't parse source".to_string()); }
    }
}
...
#[derive(Clone, Debug)]
struct Context {
    theorems: Vec<Formula>, goals: Vec<Formula>, scope: Scope
}
        ...
        match operation {
            ...
            Operation::ModusPonens(theorem_key) => {
                ...
                if let Formula::Imply(a, b) = theorem.clone() {
                    ...
                    self.goals.push(*a);
                    Ok("done".to_string())
                } else { Err(format!("outermost Formula is not an Implication ({})", theorem.description()).to_string()) }
            }
            Operation::Name(values) => {
                ...
                for value in values.into_iter() {
                    if let Formula::ThereExist(variable, formula) = goal {
                        ...
                        return Err(format!("outermost Formula is not ThereExist ({})", goal.description()).to_string());
                    }
                }
            ...
            Operation::QED => {
                ...
                if goal == TRUE { Ok("∎".to_string()) }
                else { self.goals.push(goal); Ok("goal is not ⊤".to_string()) }
            }
